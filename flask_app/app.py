...
def execute_command(host, command):
    """
    Execute a command via SSH on a given host.
    ...
    """
    ip_address = hosts.get(host, host)  # Use IP if defined, otherwise fallback to hostname
    ssh_command = ["ssh", f"sftp@{ip_address}", command]
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Failed to connect to {ip_address}: {result.stderr.strip()}")
